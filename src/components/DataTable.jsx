...
export default function DataTable({ currency }){
    ...
    useEffect(()=>{
       const getData = async () => {
           ...
           setShowItems(data.map( item => <TableRow key={item.id} coinInfo={item} currency={currency} /> ))
           setLoading(false)
       }  
       getData();
    },[currency])
    ...
    return (
        <section className="py-1 duration-75  bg-gray-100 dark:bg-gray-800">
            <div className="container lg:w-12/14 xl:w-9/12 2xl:w-8/12 mb-8 xl:mb-0 px-4 mx-auto mt-8" >
                <input type="text" placeholder="What are you looking for?" className="duration-75 input light:input-bordered  w-full dark:bg-[#2A3441]"/>
            </div>
