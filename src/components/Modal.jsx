...
const formatAddress = (accountAddress) => {
    return accountAddress.slice(0,5)+'...'+accountAddress.slice(-4)
}
        ...
        <>
            ...
            <label for="my-modal-4" className="modal modal-bottom sm:modal-middle cursor-pointer duration-300 ">
                <label className="modal-box relative bg-gray-100 dark:bg-gray-900">
                    ...
                    { loading ?
                        <Spinner/> :
                        <div className="flex gap-4 rounded-xl mt-5 p-2 items-center cursor-pointer hover:bg-base-200 dark:hover:bg-gray-800 transition duration-200 ease-in-out " onClick={onLoginHandler}>
                            ...
                            <p className="text-md font-bold text-center select-none leading-5"> 
                                { accountAddress ? formatAddress(accountAddress) : provider ? "Connect with Metamask" : "Click Here to Install Metamask"}
                            </p>                    
                        </div>
                    }
                </label>
